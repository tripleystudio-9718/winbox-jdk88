...
// Updated image loading function for the slot21_part images
const getSlotImageUrl = (number) => {
  try {
    // Using the slot21_part folder and correct naming convention
    return new URL(`/src/assets/slot21_part/slot21_${number}.png`, import.meta.url).href;
  } catch (error) {
...
// Tab configuration - only 2 tabs as requested
const tabs = [
  { id: 'SLOT GAME', name: 'SLOT GAME' },
  { id: 'TEMBAK IKAN', name: 'TEMBAK IKAN' }
];

// Active tab state - initialized to match first tab ID
...
// Initialize all game collections with slot21_part images
onMounted(() => {
  // Initialize SLOT games with images from slot21_1.png to slot21_111.png
