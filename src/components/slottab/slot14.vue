<script setup>
    ...
    // Updated image loading function for the slot14_part images
    const getSlotImageUrl = (number) => {
      try {
        // Using the slot14_part folder and correct naming convention
        return new URL(`/src/assets/slot14_part/slot14_${number}.png`, import.meta.url).href;
      } catch (error) {
    ...
    const tabs = [
      ...
      { id: 'APP DOWNLOAD', name: 'APP DOWNLOAD' },
    ];
    ...
    const table = ref([]);
    const other = ref([]);
    const videopoker = ref([]);
    const appdownload = ref([]);
    
    // Initialize all game collections with slot14_part images
    onMounted(() => {
      // Initialize SLOT games with images from slot14_1.png to slot14_666.png
      ...
      fishing.value = fishingCollection;
      ...
      for (let i = 920; i <= 948; i++) {
        otherCollection.push({
          img: getSlotImageUrl(i),
          alt: `Other Game ${i}`
        });
      }
      other.value = otherCollection;
    
      // Initialize VIDEO POKER games
      const videopokerCollection = [];
      for (let i = 950; i <= 953; i++) {
        videopokerCollection.push({
          img: getSlotImageUrl(i),
          alt: `VideoPoker Game ${i}`
        });
      }
      videopoker.value = videopokerCollection;
    
      // Initialize APP DOWNLOAD games
      const appdownloadCollection = [];
      for (let i = 960; i <= 960; i++) {
        appdownloadCollection.push({
          img: getSlotImageUrl(i),
          alt: `Appdownload Game ${i}`
        });
      }
      appdownload.value = appdownloadCollection;
    });
    ...
    <template>
      <div>
        ...
        <div class="game-container">
          ...
          <!-- Game Grid - FISHING -->
          <div v-if="activeTab === 'FISHING'" class="games-grid">
            <div v-for="(game, index) in fishing" :key="'fishing-'+index" class="game-card">
              <div class="game-image">
                <img 
                  :src="game.img" 
                  :alt="game.alt"
          ...
          <!-- Game Grid - TABLE -->
          <div v-if="activeTab === 'TABLE'" class="games-grid">
          ...
          <!-- Game Grid - OTHER -->
          <div v-if="activeTab === 'OTHER'" class="games-grid">
            <div v-for="(game, index) in other" :key="'other-'+index" class="game-card">
              <div class="game-image">
                <img 
                  :src="game.img" 
                  :alt="game.alt"
          ...
          <!-- Game Grid - VIDEO POKER -->
          <div v-if="activeTab === 'VIDEO POKER'" class="games-grid">
            <div v-for="(game, index) in videopoker" :key="'videopoker-'+index" class="game-card">
              <div class="game-image">
                <img 
                  :src="game.img" 
                  :alt="game.alt"
          ...
          <!-- Game Grid - APP DOWNLOAD -->
          <div v-if="activeTab === 'APP DOWNLOAD'" class="games-grid">
            <div v-for="(game, index) in appdownload" :key="'appdownload-'+index" class="game-card">
              <div class="game-image">
                <img 
                  :src="game.img" 
                  :alt="game.alt"
