...
// Updated image loading function for the slot24_part images
const getSlotImageUrl = (number) => {
  try {
    // Using the slot24_part folder and correct naming convention
    return new URL(`/src/assets/slot24_part/slot24_${number}.png`, import.meta.url).href;
  } catch (error) {
...
const tabs = [
  { id: 'SLOT', name: 'SLOT' },
  { id: 'TABLE GAME', name: 'TABLE GAMES' },
  { id: 'ARCADE', name: 'ARCADE' },
  { id: 'HOT GAMES', name: 'HOT GAMES' },
];

// Active tab state - set default to first tab
...
// Initialize all game collections with slot16_part images
onMounted(() => {
  // Initialize SLOT games with images from slot16_1.png to slot16_134.png
  ...
  for (let i = 500; i <= 562; i++) {
    tableCollection.push({
      img: getSlotImageUrl(i),
      alt: `TABLE Game ${i}`
    });
  }
  tablegame.value = tableCollection;
  ...
  // Initialize HOT GAMES - using a mix of popular games from different categories
  const hotGamesCollection = [];
  // Adding some top games from each category
  const topGameIndices = [
    1, 5, 10, 15, 20, 25,  // Some top slot games
    200, 205, 210,         // Some top arcade games
    500, 510, 520          // Some top table games
  ];
  
  for (const i of topGameIndices) {
    hotGamesCollection.push({
      img: getSlotImageUrl(i),
      alt: `Hot Game ${i}`
    });
  }
  ...
  <div>
    ...
    <div class="game-container">
      ...
      <div v-if="activeTab === 'TABLE GAME'" class="games-grid">
        <div v-for="(game, index) in tablegame" :key="'table-'+index" class="game-card">
          <div class="game-image">
            <img 
              :src="game.img" 
              :alt="game.alt"
      ...
      <!-- Game Grid - HOT GAMES -->
      <div v-if="activeTab === 'HOT GAMES'" class="games-grid">
        <div v-for="(game, index) in hotgames" :key="'hotgame-'+index" class="game-card">
          <div class="game-image">
            <img 
              :src="game.img" 
              :alt="game.alt"
