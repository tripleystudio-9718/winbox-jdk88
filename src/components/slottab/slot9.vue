...
// Updated image loading function for the slot9_part images
const getSlotImageUrl = (number) => {
  try {
    // Using the slot9_part folder and correct naming convention
    return new URL(`/src/assets/slot9_part/slot9_${number}.png`, import.meta.url).href;
  } catch (error) {
...
const slotgame = ref([]);
const ecasinogame = ref([]);
const crashgame = ref([]);
const tembakikan = ref([]);
const bingogame = ref([]);
const hotgame = ref([]);

// Initialize all game collections with slot9_part images
onMounted(() => {
  // Initialize SLOT games with images from slot9_1.png to slot9_215.png
  ...
  // Initialize ECASINO games
  const ecasinoCollection = [];
  for (let i = 300; i <= 305; i++) {
    ecasinoCollection.push({
      img: getSlotImageUrl(i),
      alt: `E-Casino Game ${i}`
    });
  }
  ecasinogame.value = ecasinoCollection;

  // Initialize CRASH games
  const crashCollection = [];
  for (let i = 400; i <= 401; i++) {
    crashCollection.push({
      img: getSlotImageUrl(i),
      alt: `CRASH Game ${i}`
    });
  }
  crashgame.value = crashCollection;

  // Initialize TEMBAK IKAN games
  const tiCollection = [];
  ...
  // Initialize HOT games
  const hotCollection = [];
  for (let i = 700; i <= 737; i++) {
    hotCollection.push({
      img: getSlotImageUrl(i),
      alt: `HOT Game ${i}`
    });
  }
  hotgame.value = hotCollection;
});
...
<template>
  <div>
    ...
    <div class="game-container">
      ...
      <!-- Game Grid - E-CASINO -->
      <div v-if="activeTab === 'E-CASINO'" class="games-grid">
        <div v-for="(game, index) in ecasinogame" :key="'ecasino-'+index" class="game-card">
          <div class="game-image">
            <img 
              :src="game.img" 
              :alt="game.alt"
      ...
      <!-- Game Grid - CRASH -->
      <div v-if="activeTab === 'CRASH'" class="games-grid">
        <div v-for="(game, index) in crashgame" :key="'crash-'+index" class="game-card">
          <div class="game-image">
            <img 
              :src="game.img" 
              :alt="game.alt"
      ...
      <div v-if="activeTab === 'TEMBAK IKAN'" class="games-grid">
        <div v-for="(game, index) in tembakikan" :key="'tembak-'+index" class="game-card">
          <div class="game-image">
            <img 
              :src="game.img" 
              :alt="game.alt"
      ...
      <!-- Game Grid - BINGO -->
      <div v-if="activeTab === 'BINGO'" class="games-grid">
      ...
      <div v-if="activeTab === 'HOT'" class="games-grid">
        <div v-for="(game, index) in hotgame" :key="'hot-'+index" class="game-card">
          <div class="game-image">
            <img 
              :src="game.img" 
              :alt="game.alt"
